...
const Category = () => {
  ...
  return (
    ...
    {data?.data.data.map(function(category, idx){return  <div key ={idx} className="w-full sm:w-full md:w-1/3 lg:w-1/3 p-4">
    
    <div className="bg-slate-100 p-3">
    <Link to={`/categoryDetails/${category._id}`}>
     <img src={category.image} className = "w-full h-[300px]" alt="" />
  
     <h3 className="mt-3 text-green-700 text-center font-bold text-2xl">{category.name}</h3>
